...
- **No giant timeline.** A page-level scene state machine (`SceneId` union) hands each scene its progress; scenes own their triggers, cleanup, reduced-motion, and mobile behavior. Pixel transitions own mode changes between backgrounds.
...
2. **Scene-state seam** — the pure page-level controller: scroll progress in → active `SceneId`, per-scene progress, background mode, and transition state out. Vitest unit tests. This is how reversibility and choreography ordering are testable without a GPU: forward and reverse progress sequences must produce mirrored state.
...
WebGL pixels are not asserted; shaders are verified through the scene-state seam (mode/uniform state) plus manual visual QA against the reference assets. Playwright tests prescribed by the brief (`lens-page.spec.ts`, `reduced-motion.spec.ts`, `content-schema.spec.ts`) map onto seams 3, 3, and 1 respectively.
